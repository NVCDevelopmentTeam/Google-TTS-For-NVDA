...
- Match the three IBMTTS-style modes at implementation-only hidden boundaries: `Do not shorten` preserves all engine PCM; `Shorten at end of text only` preserves internal boundary pauses and shortens only the final text-end pause; `Shorten all pauses` shortens internal and final pauses. Latency segmentation must not make mode 0 or mode 1 behave like mode 2.
...
- Pure pause and streaming PCM helpers live in `googleTtsForNvda/synthDrivers/googleTtsForNvda/speech_processing.py`: `PAUSE_MODE_DO_NOT_SHORTEN`, `PAUSE_MODE_SHORTEN_END_ONLY`, `PAUSE_MODE_SHORTEN_ALL`, `PcmSilenceShortener`, `PcmSilenceShortener._blockSizeSamples`, `PcmSilenceShortener._blockSizeBytes`, `PcmSilenceShortener._pendingBlock`, `PcmSilenceShortener._process_block()`, `PcmSilenceShortener._flush()`, `PcmSilenceShortener._hold_silence()`, `PcmSilenceShortener._release_held_silence()`, `PcmSilenceShortener.feed()`, `PcmSilenceShortener.flush_boundary()`, `PcmSilenceShortener.finish()`, `PcmLeadBuffer`, `PcmLeadBuffer.feed()`, `PcmLeadBuffer.finish()`, `create_pcm_silence_shortener()`, `pcm_bytes_for_milliseconds()`, `align_pcm_bytes()`, and `pcm_has_audible_sample()`. This module must remain importable without NVDA.
